...
export default {
  ...
  data() {
    return {
      containerActive: false, // 拖动滑块时的样式
      containerSuccess: false, // 验证成功时的滑块容器的样式
      containerFail: false, // 验证失败时的滑块容器的样式
      canvasCtx: null,
      blockCtx: null,
      block: null,
      block_x: undefined,
  ...
  methods: {
    init() {
      this.initDom(); // 获取对应的DOM
      this.initImg(); // 初始化背景图片及拼图
      this.bindEvents(); // 绑定事件
    },
    initDom() {
      this.block = this.$refs.block; // 获取 拼图元素
      this.canvasCtx = this.$refs.canvas.getContext("2d"); // 背景图片canvas元素的绘制上下文
      this.blockCtx = this.block.getContext("2d"); // 拼图的canvas元素的绘制上下文
    },
    initImg() {
    ...
    sliderDown(event) {
      /**
       * 按下滑块时,将滑块的x,y坐标记录,保存在originX和originY中
       */
    ...
    bindEvents() {
      ...
      document.addEventListener("mousemove", (e) => {
        ...
        this.trail.push(moveY); // 记录拖动滑块过程中的y方向的鼠标偏移情况
      });
      /**
       * 监听鼠标弹起事件
       *   调用verfiy进行校验
      ...
      document.addEventListener("mouseup", (e) => {
        ...
        if (spliced) {
          if (TuringTest) {
            ...
            this.$emit("success"); // 触发自定义的success事件,父元素监听该事件即可绑定验证成功时的回调
          } else {
          ...
          this.containerFail = true;
          this.$emit("fail"); // 触发自定义的fail事件,父元素监听该事件即可绑定验证失败时的回调
          setTimeout(() => {
            this.reset();
          }, 1000);
        }
    ...
    verify() {
       ...
       * 判断滑块验证过程是否为人工验证
       *
       * this.trail - 记录了拖动滑块过程中鼠标的y坐标
       * average: 拖动滑块过程中鼠标y坐标的平均值
       * deviations: 拖动滑块过程中每个位置下,鼠标y坐标相对平均值的偏离
       * 计算y坐标值的标准差
       */
      ...
      return {
        ...
        TuringTest: average !== stddev, // 这两个值如果相等,表明滑动滑块过程中,鼠标的y坐标一直未变化,为一条标准的直线,说明非人为验证
      };
    },
    reset() {
      this.containerActive = false;
